...
/// Spawn a child process on the PTY using posix_spawnp with an explicit argv.
/// `argv_bytes` is a MoonBit Bytes containing `argc` null-terminated strings
/// concatenated together (e.g., "prog\0arg1\0arg2\0").
/// The first string is used as the executable path (searched in $PATH via
/// posix_spawnp if it does not contain a slash).
/// Returns the child PID on success, -1 on failure.
///
/// Design rationale: Same posix_spawn approach as shimux_pty_spawn to avoid
/// fork-related issues with the MoonBit runtime. This variant accepts an
/// explicit argv array instead of wrapping the command with /bin/sh -c.
/// Uses posix_spawnp (not posix_spawn) so that bare command names like "bash"
/// are resolved via $PATH, matching the behavior users expect from a shell.
#[unsafe(no_mangle)]
pub extern "C" fn shimux_pty_spawn_argv(handle: i32, argc: i32, argv_bytes: *const u8) -> i32 {
    use crate::bytes_length;

    if argc <= 0 || argv_bytes.is_null() {
        return -1;
    }

    // Parse null-terminated strings from argv_bytes
    let total_len = bytes_length(argv_bytes) as usize;
    if total_len == 0 {
        return -1;
    }
    let raw = unsafe { std::slice::from_raw_parts(argv_bytes, total_len) };

    // Split by null bytes to extract argc strings
    let mut args: Vec<Vec<u8>> = Vec::new();
    let mut start = 0;
    for i in 0..total_len {
        if raw[i] == 0 {
            let mut s = Vec::with_capacity(i - start + 1);
            s.extend_from_slice(&raw[start..i]);
            s.push(0); // null-terminate for C
            args.push(s);
            start = i + 1;
    ...
    // Handle last segment if no trailing null
    if args.len() < argc as usize && start < total_len {
        let mut s = Vec::with_capacity(total_len - start + 1);
        s.extend_from_slice(&raw[start..total_len]);
        s.push(0);
        args.push(s);
    }
    ...
    // Reset SIGCHLD to default so waitpid works.
    {
        let mut sa: libc::sigaction = unsafe { std::mem::zeroed() };
        sa.sa_sigaction = libc::SIG_DFL;
        sa.sa_flags = 0;
    ...
    const POSIX_SPAWN_SETSID: libc::c_short = 0x0400;
    #[cfg(target_os = "linux")]
    const POSIX_SPAWN_SETSID: libc::c_short = 0x02;
    ...
    let mut argv_ptrs: Vec<*mut libc::c_char> = args.iter()
    ...
    let exe_path = args[0].as_ptr() as *const libc::c_char;

    let mut child_pid: libc::pid_t = 0;
    // Design rationale: posix_spawnp (not posix_spawn) is used here so that
    // bare command names (e.g. "bash") are resolved via $PATH. posix_spawn
    // requires an absolute path and would fail with ENOENT for bare names.
    let spawn_ret = unsafe {
        libc::posix_spawnp(
            &mut child_pid,
            exe_path,
